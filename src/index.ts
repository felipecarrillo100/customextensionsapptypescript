                ...
                if (typeof callback === "function") {
                    if (o.features) {
                        if (o.features.length==2) {
                            ...
                            const newCommand = TomTomAPIURL([point1.y, point1.x], [point2.y, point2.x], 'Tomtom route');
        ...
        },
        {
            ...
            transform: (dataStr: string) => {
                ...
                if (tomtomResponse.routes.length > 0 && tomtomResponse.routes[0].legs.length > 0) {                
                    const coordinates = tomtomResponse.routes[0].legs[0].points.map((point:any)=>[point.longitude, point.latitude]);
...
const SampleCommand = {
  ...
  const TomTomAPIURL = (point1: number[], point2: number[], label: string) => ({
    "action": 10,
    "parameters": {
      "action": "MemoryFeatureLayer",
      "autozoom": true,
